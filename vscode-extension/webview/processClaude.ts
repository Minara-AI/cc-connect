...
import { stripSystemWrappers } from './promptWrappers';
...
export function processClaude(
  ...
  for (const raw of events) {
    const ev = raw as RawEvent & { text?: string };
    const t = ev.type;
    const sub = ev.subtype;

    if (t === 'cc:user-prompt' && typeof ev.text === 'string') {
      blocks.push({ kind: 'prompt', text: ev.text });
      continue;
    }
    ...
    if (t === 'user' && typeof ev.message?.content === 'string') {
      ...
      const cleaned = stripSystemWrappers(ev.message.content);
