...
impl TryFrom<PrivateKeyInfoRef<'_>> for KeypairBytes {
    ...
    fn try_from(private_key: PrivateKeyInfoRef<'_>) -> Result<Self> {
        ...
        // - 0x20: 32-byte length
        // pkcs8::Error::KeyMalformed is a tuple variant `KeyMalformed(KeyError)`;
        // bare `Error::KeyMalformed` would be a `fn(KeyError) -> Error` function
        // pointer rather than an `Error` value. Pass `KeyError::Invalid` to the
        // tuple variant so the closure returns `Error` and `?` can convert.
        let secret_key = match private_key.private_key.as_bytes() {
            [0x04, 0x20, rest @ ..] => rest.try_into().map_err(|_| Error::KeyMalformed(KeyError::Invalid)),
            _ => Err(Error::KeyMalformed(KeyError::Invalid)),
        }?;

        let public_key = private_key
            .public_key
            .and_then(|bs| bs.as_bytes())
            .map(|bytes| bytes.try_into().map_err(|_| Error::KeyMalformed(KeyError::Invalid)))
            .transpose()?
            .map(PublicKeyBytes);
